...
private:
    ...
    public:
        ...
        size_t mysqlISO8601Time(char * dest, Time source, UInt64, UInt32, const DateLUTImpl & timezone)
        {
            writeNumber2(dest, ToHourImpl::execute(source, timezone));
            writeNumber2(dest + 3, ToMinuteImpl::execute(source, timezone));
...
public:
    ...
    template <typename T>
    size_t parseFormat(const String & format, std::vector<Action<T>> & instructions, UInt32 scale, bool only_fixed_width_formatters, String & out_template) const
    {
        ...
        else if constexpr (format_syntax == FormatSyntax::Joda)
    ...
    template <typename T>
    size_t parseMySQLFormat(const String & format, std::vector<Action<T>> & instructions, UInt32 scale, bool only_fixed_width_formatters, String & out_template) const
    {
